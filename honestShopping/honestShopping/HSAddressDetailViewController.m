...
- (void)deldelAddressRequestWithUid:(NSString *)uid sessionCode:(NSString *)sessionCode add_id:(NSString *)add_id
{
    ...
    [self.httpRequestOperationManager POST:kDelAddressURL parameters:[HSPublic apiPostParas:parametersDic]success:^(AFHTTPRequestOperation *operation, id responseObject) { /// 失败
...
- (void)defaultAddressRequestWithUid:(NSString *)uid sessionCode:(NSString *)sessionCode add_id:(NSString *)add_id
{
    ...
    [self.httpRequestOperationManager POST:kSetDefaultAddressURL parameters:[HSPublic apiPostParas:parametersDic] success:^(AFHTTPRequestOperation *operation, id responseObject) { /// 失败
        [self hiddenHudLoading];
    } failure:^(AFHTTPRequestOperation *operation, NSError *error) {
        NSLog(@"%s failed\n%@",__func__,operation.responseString);
        [self hiddenHudLoading];
