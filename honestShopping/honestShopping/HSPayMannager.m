...
#pragma mark - 添加记录
- (void)addRecordWithUid:(NSString *)uid payType:(NSString *)type orderID:(NSString *)orderID price:(NSNumber *)price sessionCode:(NSString *)sessionCode
{
...
- (void)startUpdatePayRecord
{
    ...
    [record enumerateObjectsUsingBlock:^(NSDictionary *obj, NSUInteger idx, BOOL *stop) {
        ...
        NSNumber *price = obj[kPostJsonPrice];
        ...
        [self updateOrderStateRequest:orderID uid:uid price:price sessionCode:sessionCode];
    }];
    
}

- (void)updateOrderStateRequest:(NSString *)orderID uid:(NSString *)uid price:(NSNumber *)price sessionCode:(NSString *)sessionCode
{
    ...
    [_requestOperationManager POST:kUpdateOrderNextURL parameters:[HSPublic apiPostParas:parametersDic] success:^(AFHTTPRequestOperation *operation, id responseObject) { /// 失败
