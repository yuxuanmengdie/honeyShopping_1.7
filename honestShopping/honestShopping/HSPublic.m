...
+ (NSData *)aes256_encryptWithSource:(NSString *)ori  key:(NSString *)keyStr iv:(NSData *)iv//加密
{
    NSData *oriData = [ori dataUsingEncoding:NSUTF8StringEncoding];
    ...
    NSData *result = nil;
    size_t bufferSize = oriData.length + kCCBlockSizeAES128;
...
//data 转base64位
+ (NSString *)base64EncodedString:(NSData *)ori {
    if (!ori) {
        return nil;
    }
    
    NSString *result = [ori base64EncodedStringWithOptions:NSDataBase64Encoding64CharacterLineLength];
...
+ (NSString *)aes256Key:(NSString *)md5Str {
    ...
    NSString *sub1 = [md5Str substringWithRange:NSMakeRange(0, 5)];
    NSString *sub2 = [md5Str substringWithRange:NSMakeRange(9, 5)];
    NSString *sub3 = [md5Str substringWithRange:NSMakeRange(19, 6)];
    NSString *result = [NSString stringWithString:sub1];
    result = [[result stringByAppendingString:sub2] stringByAppendingString:sub3];
    
    return result;
}

//aes 加密向量iv
#define kAESIV @"1234567812345678"

//根据json参数加密成字符串 （aes256的key+加密后base64字符）
+ (NSString *)aesEncodeJsonStr:(NSString *)json {
    NSString *md5Str = [HSPublic md5Str:json];
    NSString *key = [HSPublic aes256Key:md5Str];
    NSString *base64Str = [HSPublic base64EncodedString:[HSPublic aes256_encryptWithSource:json key:key iv:[kAESIV dataUsingEncoding:NSUTF8StringEncoding]]];
    NSString *result = [NSMutableString stringWithString:md5Str];
    result = [result stringByAppendingString:base64Str];
    return result;
}

//aes加密后的报文参数
+ (NSString *)postAESPara:(NSDictionary *)postAESPara {
    if (!postAESPara) {
        return nil;
    }
    
    NSString *ori = [HSPublic dictionaryToJson:postAESPara];
    NSString *aesStr = [HSPublic aesEncodeJsonStr:ori];
    NSDictionary *signDic = @{@"sign":aesStr};
    NSString *result = [HSPublic dictionaryToJson:signDic];
    return result;
}

+ (NSDictionary *)apiPostParas:(NSDictionary *)oriPara {
    
    NSString *result = [HSPublic dictionaryToJson:oriPara];
    if (kAPIAesEncryptFlag) {
        result = [HSPublic postAESPara:oriPara];
    }
    
    NSDictionary *dic = @{kJsonArray:result};
...
+ (void)loginIn
{
    ...
    [manager POST:loginURL parameters:[HSPublic apiPostParas:parametersDic] success:^(AFHTTPRequestOperation *operation, id responseObject) { /// 失败
