...
- (void)SetUpregisterVC
{
    ...
    _registerViewController.registerSuccessBlock = ^(NSString *userName, NSString *password){
        ...
        wself.userNameTextField.text = userName;
        wself.passWordTextFiled.text = password;
    };
...
- (void)loginRequest:(NSString *)userName password:(NSString *)passWord
{
    ...
    [self.httpRequestOperationManager POST:kLoginURL parameters:[HSPublic apiPostParas:parametersDic] success:^(AFHTTPRequestOperation *operation, id responseObject) { /// 失败
...
- (void)loginInWithOtherID:(NSString *)openID type:(HSLoginType)loginType userName:(NSString *)userName headerImgURL:(NSString *)imgURL
{
    ...
    [self.httpRequestOperationManager POST:kOtherLoginURL parameters:[HSPublic apiPostParas:parametersDic] success:^(AFHTTPRequestOperation *operation, id responseObject) { /// 失败
        NSLog(@"success\n%@",operation.responseString);
        [self showHudWithText:@"登录失败"];
        [self hiddenHudLoading];
