...
- (void)AddCommentRequestWithUid:(NSString *)uid itemID:(NSString *)itemID phone:(NSString *)phone star:(float)star comment:(NSString *)comment sessionCode:(NSString *)sessionCode
{
    ...
    [self.httpRequestOperationManager POST:kAddCommentURL parameters:[HSPublic apiPostParas:parametersDic] constructingBodyWithBlock:^(id<AFMultipartFormData> formData) {
        if (isFileUpload) {
