...
//                                       delegate:self];
    ...
    NSString *shareURLStr = [NSString stringWithFormat:@"%@%@",kURLHeader,@"/index.php?g=api_test&m=Test&a=download"];//@"http://ecommerce.news.cn/index.php?g=api&m=Test&a=download";
...
- (void)requestDetailByItemID:(NSString *)itemID
{
    ...
    NSDictionary *parametersDic = @{kPostJsonKey:[HSPublic md5Str:[HSPublic getIPAddress:YES]],
                                    kPostJsonid:itemID};
    __weak typeof(self) wself = self;
    [manager POST:kGetItemByIdURL parameters:[HSPublic apiPostParas:parametersDic] success:^(AFHTTPRequestOperation *operation, id responseObject) {
...
- (void)collectItemRequestWithID:(NSString *)itemID uid:(NSString *)uid sessionCode:(NSString *)sessionCode
{
    ...
    [self.httpRequestOperationManager POST:kAddFavoriteURL parameters:[HSPublic apiPostParas:parametersDic] success:^(AFHTTPRequestOperation *operation, id responseObject) { /// 失败
...
- (void)delFavoriteReqeust:(NSString *)uid itemId:(NSString *)itemId sessionCode:(NSString *)sessionCode
{
    ...
    [self.httpRequestOperationManager POST:kDelFavoriteURL parameters:[HSPublic apiPostParas:parametersDic] success:^(AFHTTPRequestOperation *operation, id responseObject) { /// 失败
        [self hiddenHudLoading];
    } failure:^(AFHTTPRequestOperation *operation, NSError *error) {
        NSLog(@"%s failed\n%@",__func__,operation.responseString);
        _isFavoriteLoading = NO;
