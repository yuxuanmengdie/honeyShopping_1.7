...
- (void)getOrderDetailRequest:(NSString *)uid orderID:(NSString *)orderID sessionCode:(NSString *)sessionCode
{
    ...
    [self.httpRequestOperationManager POST:kGetOrderDetailURL parameters:[HSPublic apiPostParas:parametersDic] success:^(AFHTTPRequestOperation *operation, id responseObject) { /// 失败
...
//    Product *product = [self.productList objectAtIndex:indexPath.row];
    ...
    NSString *privateKey = @"";
...
#pragma mark -
#pragma mark 更新订单状态
- (void)updateOrderStatus:(NSString *)orderID uid:(NSString *)uid price:(NSNumber *)price sessionCode:(NSString *)sessionCode payType:(NSString *)type
{
    _orderUpdateCount++;
    ...
    NSDictionary *parametersDic = @{kPostJsonKey:[HSPublic md5Str:[HSPublic getIPAddress:YES]],
                                    kPostJsonUid:uid,
                                    kPostJsonOrderId:orderID,
                                    kPostJsonPrice:price,
                                    kPostJsonSessionCode:sessionCode
                                    };
    
    [self.httpRequestOperationManager POST:kUpdateOrderNextURL parameters:[HSPublic apiPostParas:parametersDic] success:^(AFHTTPRequestOperation *operation, id responseObject) { /// 失败
        NSLog(@"success\n%@",operation.responseString);
        ...
        if (operation.responseData == nil) {
            if (_orderUpdateCount <= kUpdateOrderMaxCount) { //不满最大轮询次数就继续请求
                [self updateOrderStatus:orderID uid:uid price:price sessionCode:sessionCode payType:type];
            } else { //添加到未完成列表中
                [[HSPayMannager sharePayMannager] addRecordWithUid:uid payType:type orderID:orderID price:price sessionCode:sessionCode];
            }
            return ;
        }
        NSError *jsonError = nil;
...
- (void)p_paySuccess:(NSNotification *)noti
{
    ...
    [self updateOrderStatus:[HSPublic controlNullString:_orderID] uid:[HSPublic controlNullString:_userInfoModel.id] price:[NSNumber  numberWithFloat:[_orderModel.order_sumPrice floatValue]] sessionCode:[HSPublic controlNullString:_userInfoModel.sessionCode] payType:[NSString stringWithFormat:@"%d",_payType]];
}
