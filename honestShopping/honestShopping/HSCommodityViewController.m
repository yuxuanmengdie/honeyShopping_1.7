...
- (void)adsRequestWithCid:(NSString *)cid key:(NSString *)key
{
    ...
    [self.httpRequestOperationManager POST:kGetAdsByCateURL parameters:[HSPublic apiPostParas:parametersDic] success:^(AFHTTPRequestOperation *operation, id responseObject) {
        NSLog(@"JSON: %@/n %@", responseObject,[HSPublic dictionaryToJson:parametersDic]);
        _isAdsLoding = NO;
